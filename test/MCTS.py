...
class MonteCarloTreeSearchNode():
    ...
    def simulate(self, nb_simulation, model=None):
        ...
        for _ in range(nb_simulation):
            ...
            if value is None:
                ...
                # policy, value = node_to_expand.model.predict(
                # node_to_expand.board.board_state.reshape((1, 5, 5, 10)))
                policy, value = model.predict(
                    node_to_expand.board.board_state.reshape((1, 5, 5, 10)))
                possible_policy = node_to_expand.board.get_legal_moves(
                    policy[0]).flatten()
    ...
    def best_child(self, c_param=0.8):
        ...
        for action, child in self.children.items():
            score = -child.mean_value() + c_param * child.prior * \
                np.sqrt(child.parent._number_of_visits) / \
    ...
    def __repr__(self):
        ...
        return "{} Prior: {} Count: {} Value: {}".format(
            self.board.__str__(), prior, self._number_of_visits, self.mean_value())
